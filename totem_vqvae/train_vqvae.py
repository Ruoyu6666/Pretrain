...
def train_model(model, device, vqvae_config, save_dir, logger, args):
    ...
    for epoch in range(int((vqvae_config['num_training_updates']/len(train_loader)) + 0.5)):
        ...
        for i, (batch_x) in enumerate(train_loader):
            ...
            tensor_all_data_in_batch = batch_x.detach().clone().to(device=device, dtype=torch.float)
...
def create_datloaders(batchsize=100, dataset="dummy", base_path='dummy'):
    ...
    test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=batchsize,
                                                shuffle=False, num_workers=10, drop_last=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_init_cpu_or_gpu', type=str, required=False, help='the data initialization location')
    parser.add_argument('--comet_log', action='store_true', required=False, help='whether to log to comet online')
    ...
    parser.add_argument('--comet_name', type=str, required=False, help='the experiment name to add to comet')
