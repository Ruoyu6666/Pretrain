...
class Dataset(CachedDataset):
    ...
    Caching is possible if you need to avoid processing the data every time you run the script. 
    The cache file will be saved in `cache_path` and will be loaded if `cache` is set to True. 
    Be careful when using the cache, as it will not be updated if the data changes. 
    Only use once the data processing pipeline is finalized. Deleteing the cache file will force the data to be processed again.
